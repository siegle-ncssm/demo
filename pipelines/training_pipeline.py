...
from src.models.pytorch_model import PyTorchModelTrainer
...
from src.models.tensorflow_model import AdvancedKerasModel
from src.models.tensorflow_model import create_custom_model as create_tf_model
...
class MLTrainingPipeline:
    ...
    def _process_data(self) -> dict[str, pd.DataFrame]:
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X, y, test_size=data_config.get("test_size", 0.2), random_state=42
        )
    ...
    def _train_pytorch_model(self, data: dict[str, np.ndarray], config: dict[str, Any]):
        ...
        model = create_pytorch_model(
            input_dim=data["X_train"].shape[1], output_dim=1, config=config
        )
    ...
    def _train_tensorflow_model(
        self, data: dict[str, np.ndarray], config: dict[str, Any]
    ):
        """Train TensorFlow model."""
        tf_config = {
    ...
    def _create_ensemble(self, data: dict[str, np.ndarray], task: str):
        ...
        if len(base_models) >= 2:
            ...
            self.models["ensemble"] = {"model": ensemble, "builder": ensemble_builder}
    ...
    def _evaluate_models(
        ...
        for name, model_info in self.models.items():
            ...
            try:
                ...
                # Log to MLflow
                mlflow.log_metrics(
                    {f"{name}_mse": mse, f"{name}_rmse": rmse, f"{name}_r2": r2}
                )
    ...
    def _select_best_model(
        ...
        mlflow.log_params(
            {"best_model_name": best_name, "best_model_score": best_score}
        )
    ...
    def _register_model(self, best_model: dict[str, Any]) -> str:
        ...
        if best_model["name"] == "pytorch":
            ...
            mlflow.sklearn.log_model(
                pipeline.pipeline, "model", registered_model_name=model_name
            )
    ...
    def _generate_report(self, results: dict[str, dict[str, float]]) -> dict[str, Any]:
        ...
        report_file = (
            report_dir
            / f"training_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.yaml"
        )
