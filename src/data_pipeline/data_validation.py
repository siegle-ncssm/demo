...
class DataValidator:
    ...
    def _setup_context(self, root_dir: Optional[str]) -> BaseDataContext:
    ...
    def validate_data_quality(
        ...
        if 'max_duplicate_percentage' in config:
            dup_check = self._check_duplicates(
                df, config['max_duplicate_percentage']
            )
        ...
        if 'value_ranges' in config:
            range_check = self._check_value_ranges(
                df, config['value_ranges']
            )
        ...
        if 'allowed_values' in config:
            cat_check = self._check_categorical_values(
                df, config['allowed_values']
            )
    ...
    def _check_categorical_values(
        ...
        for col, allowed_vals in allowed.items():
            ...
            if invalid_vals:
                check['passed'] = False
                logger.warning(
                    f"Column '{col}' has invalid values: {invalid_vals}"
                )
